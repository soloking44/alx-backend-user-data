...
def users() -> str:
    ...
    outputs end-point to register a user
    """
...
def login():
    ...
    outputs request with form data with email and password fields
    """
...
def logout():
    ...
    get user with needed session ID, if exists, destroy session
...
def profile() -> str:
    """GET /profile
    output 403 if session ID is invalid
...
def get_reset_password_token_route() -> str:
    ...
    outputs 403 status code if email not registered
    create token to respond with 200 HTTP status if exist
    """
    user_request = request.form
    user_email = user_request.get('email', '')
...
def update_password() -> str:
    ...
    outputs a 403 HTTP code if token is invalid
